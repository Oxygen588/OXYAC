...
function checkCar(car)
	if car then
		...
		if isCarBlacklisted(carModel) then
            _DeleteEntity(car)
			TriggerServerEvent("OxyAc:Cars", blacklistedCar )
        end
	end
end
...
Citizen.CreateThread( function()
	...
	Wait(30000)
	local sansanu = 0
    local pegard = GetEntityHeightAboveGround(GetPlayerPed(-1))
	...
	while true do
		...
		if car == 0 then
				Wait(1500)
               		local pegard2 = GetEntityHeightAboveGround(GetPlayerPed(-1))
			local este = math.floor(pegard - pegard2)
                	local speed2 = GetEntitySpeed(GetPlayerPed(-1))
                	if pegard > 10 then
				if este  >= -1 and este  <= 1 then
					sansanu = sansanu + 1
				end
				if sansanu == 2 then
					TriggerServerEvent("WarnPNoclip", Warn )
				end

				if sansanu == 20 then
					TriggerServerEvent("Noclipban", Ban )
				end
			end
		end
...
--antispeedhack done BaniInfiniti
Citizen.CreateThread( function()
	local pingi = 1
	local sansanu = 0
        SetBlipSprite(5,137)
	...
	while true do
		...
		local kmh = (speed * 3.6);
		if car == 0 then
			if kmh > 150 and kmh < 500 then
				sansanu = sansanu + 1
			end
			if sansanu == 2 then
				TriggerServerEvent("WarnPSpeedHack", Ban )
			end

			if sansanu == 15 then
				TriggerServerEvent("BanPlayerSpeedH", Ban )
			end
...
Citizen.CreateThread( function()
	...
	while true do
			...
			if IsPedArmed(player, 7) then
				...
				local mataba = IsPedShooting(Ped)
				if ammoTotal > 5000 then
					TriggerServerEvent("BanPlayerINFAMMO", BanPlayer )
...
Citizen.CreateThread(function()
	Wait(30000)
	while true do
	local viatainainte2 = GetEntityHealth(GetPlayerPed(-1))
	local viatainainte = GetEntityHealth(GetPlayerPed(-1))
	if viatainainte == 120 then 
		SetEntityHealth(GetPlayerPed(-1), (viatainainte2-1))
	else
		SetEntityHealth(GetPlayerPed(-1), (viatainainte-1))
	end
	local viatainainte2 = GetEntityHealth(GetPlayerPed(-1))
	Wait(1001)
...
Citizen.CreateThread(function()
	...
	while true do
    local viatainainte2 = GetEntityHealth(GetPlayerPed(-1))
	if viatainainte2 == 120 then
		SetEntityHealth(GetPlayerPed(-1), (viatainainte2-1))
	else
		SetEntityHealth(GetPlayerPed(-1), (viatainainte2+1))
	end
    	if viatainainte2 == 200 then 
		sansanu = sansanu + 1
	end
	if sansanu == 5 then

		TriggerServerEvent("WarnPGodmod", Warn )
	end
	if sansanu == 10 then
		TriggerServerEvent("WarnPGodmod", Warn )
	end
	...
	if sansanu == 20 then
		TriggerServerEvent("BanPlayerGODMODE", BanPlayer )
	end
...
Citizen.CreateThread(function()
	...
	while true do
    local viatainainte2 = GetEntityHealth(GetPlayerPed(-1))
	if viatainainte2 == viatainainte2 then
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsPedJumping(PlayerPedId()) then
			...
			if jumplength > 115 then
				sansanu = sansanu + 1
			end
			if sansanu == 2 then
				TriggerServerEvent("MegajumpWarn", MegajumpWarn )
			end

			if sansanu == 5 then
				TriggerServerEvent("BanPlayerJump")
			end
